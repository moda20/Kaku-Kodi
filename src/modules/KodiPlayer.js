...
function serve_directory(directory, port) {
    ...
    var server = http.createServer(function(req, res) {
        var done = finalhandler(req, res);e
        serve(req, res, done);
    });
...
function PlayNow(filepath,server_ip,server_port){
   return new Promise((res,rej)=>{
       try{
           ...
           const server_ip = args.server
           const server_port = args.port*/
           console.log(filepath,server_ip,server_port);
           ...
           if (filepath.indexOf('://') == -1) {
               ...
               serve_directory(directory, local_port);
               let request =kodi_post(network_file, server_ip, server_port);
